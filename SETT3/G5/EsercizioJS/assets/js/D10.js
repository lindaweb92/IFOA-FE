...
function whoIsBigger (numeri) {
  const maggiore = Math.max(...numeri);
  return{
    maggiore
  };
}
const arrayNum = [10, 5];
const risultato = whoIsBigger(arrayNum);
console.log("Maggiore:", risultato.maggiore);
...
function deleteOne(str,bool){
  if(bool == true) return str.slice(-3)
...
function onlyLetters(str2){
  ...
  console.log(str2.slice(0,6)+str2.slice(8))
}
...
const giornoCorrente = whatDayIsIt.getDay();
...
*/
function howManyDays(data1, data2) {
...
const dataInizio = new Date(2023, 10, 1);
const dataFine = new Date(2023, 10, 10);
const differenzaGiorni = howManyDays(dataInizio, dataFine);
...
const dataNascita = new Date(1992, 6, 3);
const eta = isTodayMyBirthday(dataNascita);
console.log("Età:", eta);
...
*/
function newestMovie(movies,Year) {
  if (Year > 2016) {
